...
export default function EventDetail() {
  ...
  useEffect(() => {
    api.get(`/events/${id}/`).then(r => setEvent(r.data)).catch(() => setError('Event not found')).finally(() => setLoading(false));
  }, [id]);
  ...
  return (
    <div data-testid="event-detail-page">
      ...
      {event.description && (
        <div className="mt-6 text-sm text-gray-700 leading-relaxed whitespace-pre-wrap" data-testid="event-description">
          {event.description}
        </div>
      )}
      ...
      <div className="mt-8 pt-6 border-t border-gray-200">
        {event.my_signup_status ? (
          <div className="flex items-center gap-2">
            ...
            }`} data-testid="event-signup-status">
              {event.my_signup_status}
            </span>
          </div>
        ) : (
